...
from sqlalchemy import Table, Column, String, Text, DateTime, ForeignKey, Integer, Sequence
...
concepts2schemes = Table('concepts2schemes', Base.metadata,
    ...
    Column('concept_uri', String(255), ForeignKey('object.uri'))
)
...
concept_properties_lg = Table('concept_properties_lg', Base.metadata,
    Column('concept_uri', String(255), ForeignKey('concept.uri')),
    Column('property_type', String(255)),
    Column('property_value', String(500)),
    Column('property_lg', String(50))
)
...
"""
  Create a simple collection Class
  with special method for the properties with lg
"""
class AssociatedPropertiesLgs(object):
  def __init__(self):
      self.data = []
  ...
  def getAssociatedPropertiesLg(self, propertyType, propertyLg='en'):
    matchedpr = []
    for pr in self.data:
  ...
  def getAssociatedPropertiesLgValue(self, propertyType, propertyLg='en', separator = ','):
    matchedpr = []
    for pr in self.data:
      if (pr.propertyType in propertyType) and (pr.propertyLg == propertyLg) : 
...
class AssociatedPropertiesLg(Base):
  __tablename__ = 'associated_properties_lg'
  id = Column(Integer, Sequence('user_id_seq'), primary_key=True)
  uri = Column(String(255), ForeignKey('object.uri'))
  propertyType = Column( String(255))
  propertyValue = Column( String(500))
  propertyLg = Column( String(50))
  
  def __init__(self, conceptUri, propertyType, propertyValue=None, propertyLg='en'):
    self.uri = conceptUri
    self.propertyType = propertyType
    self.propertyValue = propertyValue
    self.propertyLg = propertyLg
        
  def __repr__(self):
    return "<skos:%s xml:lang='%s'>%s</skos:%s>" % (self.propertyType.encode('utf-8'), self.propertyLg.encode('utf-8'), self.propertyValue.encode('utf-8'), self.propertyType.encode('utf-8'))
...
class Concept(Object):
    ...
    isTopConcept = Column(String(50))
    hasTopConcept = Column(String(250))
    ...
    # many to one Concept <-> Concept representing properties with lang <->
    associatedPropertiesLg = relationship('AssociatedPropertiesLg',  collection_class=AssociatedPropertiesLgs, backref=backref('concept', uselist=False, collection_class=AssociatedPropertiesLgs))
...
class ConceptScheme(Object):
    ...
    # many to one Concept <-> Concept representing properties with lang <->
    associatedPropertiesLg = relationship('AssociatedPropertiesLg',  collection_class=AssociatedPropertiesLgs, backref=backref('scheme', uselist=False, collection_class=AssociatedPropertiesLgs))
...
class Collection(Object):
    ...
    # many to one Concept <-> Concept representing properties with lang <->
    associatedPropertiesLg = relationship('AssociatedPropertiesLg',  
      collection_class=AssociatedPropertiesLgs, backref=backref('collection', uselist=False, collection_class=AssociatedPropertiesLgs)
    )
...
class RDFLoader(collections.Mapping):
  ...
  def _loadConcepts(self, graph, cache):
    ...
    for subject in self._iterateType(graph, 'Concept'):
        ...
        cache[uri].isTopConcept = self._subjectIsTopConcept(graph, subject)
        cache[uri].hasTopConcept = self._getHasTopConcept(graph, subject)
        self._getAssociatedPropertiesLg(graph, subject, cache)
  ...
  def _loadConceptSchemes(self, graph, cache):
      ...
      ##### ADD load concepts for the schema !!!! I don't understand why it's missing #######
      for subject, object_ in graph.subject_objects(predicate=rdflib.URIRef('http://www.w3.org/2004/02/skos/core#inScheme')):
        try:
  ...
  def _getHasTopConcept(self, graph, subject):
    lastTerm = rdflib.URIRef(subject)
    for row in graph.query(self.qHasTopConcept, initBindings={'lastTerm': subject}):
      return row
    
  def _subjectIsTopConcept(self,graph, subject):
    obj= URIRef('http://www.w3.org/2004/02/skos/core#%s' % 'hasTopConcept')
    if graph.value(subject=None, predicate=obj, object=subject, default=None, any=False):
      return 'yes'
    else : 
      return 'no'

  def _getAssociatedPropertiesLg(self,graph, subject, cache):
    uri = self.normalise_uri(subject)
    attrsLg = {
      rdflib.URIRef('http://www.w3.org/2004/02/skos/core#altLabel'): 'altLabel',
      rdflib.URIRef('http://www.w3.org/2004/02/skos/core#prefLabel'): 'prefLabel',
      rdflib.URIRef('http://www.w3.org/2004/02/skos/core#definition'): 'definition',
    }
    langfilterAll = lambda l: True
    for predicate, attrsLg in attrsLg.iteritems():
        for object_ in  filter(langfilterAll, graph.objects(subject, predicate)):
            try:
              if type(object_).__name__ == 'Literal':
                propertyLg = AssociatedPropertiesLg(uri, attrsLg, object_.value, object_.language )
                cache[uri].associatedPropertiesLg.append(propertyLg)
              else :
                # TODO : si == URI aller chercher les valeurs de la property en question
                print repr(object_)
                print type(object_).__name__
            except KeyError:
                continue
